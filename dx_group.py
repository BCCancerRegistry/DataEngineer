...
from BCCancer.save_data import save_data
...
def clean_data(**kwargs):
    ti = kwargs["ti"]
    input_csv_location = kwargs['params']["input_csv_location"]
    ...
    date_from = format_date("2023-10-01", date_format="%Y-%m-%d")
    date_to = format_date("2023-10-01", date_format="%Y-%m-%d")
    """data = get_messages(
    ...
    data = pd.read_csv(input_csv_location)
    pipeline_name = "DX_Group"
    comment = f"Pipeline ran with parameters : {kwargs['params'].__str__()}"
    batch_row = dict(
        pipeline_name=pipeline_name, date_to=date_to, date_from=date_from, comment=comment
    )
    logger = LoggingMixin()
...
def prep_data(**kwargs):
    ...
    output_columns=['batch_id','msgid','gross','addendum','diagnosis','diagnosis_comment','micro','filtered_message','part_of_report']
    if output_db_type == "postgres":
        with PostgresConn(
                host=output_db_server,
                database_name=output_db_name,
            ...
            section_regex_columns = ["parent_category", 'nha', 'fha', 'fha2', 'iha', 'vcha1', 'vcha2']
            section_regex_table = "section_regex"
            section_df = conn.get_data(
                create_sql(table=section_regex_table, columns=section_regex_columns,
                           conditions=section_regex_condition), columns=section_regex_columns
            )
...
def predict_data(**kwargs):
    ...
    prepped_data = ti.xcom_pull(task_ids="data_prep_task", key="output_table")['output_table']
    output_db_type = kwargs["params"]["output_db_type"]
    output_db_name = kwargs["params"]["output_db_name"]
    output_db_server = kwargs["params"]["output_db_server"]
    output_db_port = kwargs["params"]["output_db_port"]
    ...
    if output_db_type == "postgres":
        with PostgresConn(
            ...
            data[["predicted_label", "model_score"]] = model.apply_model(
                final_column
            )
            data['predicted_label'] = data['predicted_label'].apply(lambda x: x.split('_')[1])
...
# DAG definition
with DAG(
        "DX_Group_classification_with_csv",
        default_args=default_args,
        schedule_interval=None,
        params={
            "model_path": Param(default="null", type=["string", "null"]),
            "model_name": Param(default="clinicalBERT_16class_onco_Dxgroup_10k", type="string"),
            "input_csv_location": Param(default="/data/hl7msgs.csv", type='string'),
            "model_version": Param(default=1, type="integer"),
            "model_tokenizer": Param(default="/data/models/Bio_ClinicalBERT", type="string"),
            "output_db_server": Param(type="string", default="10.42.121.61"),
            "output_db_name": Param(type="string", default='BCCancer_DE'),
            "output_db_type": Param(type="string", enum=["sql-server", "postgres"], default="postgres"),
